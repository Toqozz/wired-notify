...
pub fn init_bus(sender: mpsc::Sender<Message>) -> Connection {
    ...
    match reply {
        ...
        RequestNameReply::InQueue => println!(
    ...
    pub fn from_dbus(
        ...
        fn image_from_data(dbus_image: DBusImage) -> Option<DynamicImage> {
            ...
            let x = match dbus_image.channels {
                3 => ImageBuffer::from_raw(
                    dbus_image.width as u32,
                    dbus_image.height as u32,
                    dbus_image.data,
                )
                .map(DynamicImage::ImageRgb8),
                4 => ImageBuffer::from_raw(
                    dbus_image.width as u32,
                    dbus_image.height as u32,
                    dbus_image.data,
                )
        ...
        if let Some(Value::Struct(dbus_image)) = hints
            .remove("image-data")
            .or_else(|| hints.remove("image_data"))
        {
            hint_image = image_from_data(dbus_image);
        } else if let Some(Value::String(path)) =
